...
(function () {
  ...
  // ---------- 풋터 이메일/전화번호 — site-config.json 에서 동적 갱신 (모든 페이지 공통) ----------
    ...
    fetch('./site-config.json?t=' + Date.now(), { cache: 'no-store' })
      ...
      .then(function (cfg) {
        ...
        if (footerPhone && cfg.phone) {
          ...
          footerPhone.href = 'tel:' + telDigits;
