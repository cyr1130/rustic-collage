// Vercel Serverless Function — notice-data.json 을 GitHub에 커밋
// POST /api/save-notice
// Body: { password: string, items: array }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await octokit.repos.createOrUpdateFileContents({
      ...
      path,
      branch,
      message: 'chore(content): update notice via admin (' + new Date().toISOString() + ')',
      content: encoded,
      sha,
    });
    ...
    console.error('GitHub commit failed (notice):', {
      message: err.message,
      status: err.status,
