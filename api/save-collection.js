// Vercel Serverless Function — collection-data.json 을 GitHub에 커밋
...
module.exports = async function handler(req, res) {
  ...
  const path = 'collection-data.json';
  ...
  try {
    ...
    const result = await octokit.repos.createOrUpdateFileContents({
      ...
      path,
      branch,
      message: 'chore(content): update collection via admin (' + new Date().toISOString() + ')',
      content: encoded,
      sha,
    });
    ...
    console.error('GitHub commit failed (collection):', {
      message: err.message,
      status: err.status,
